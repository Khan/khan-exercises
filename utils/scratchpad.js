function Scratchpad(){
	...
	var height = Math.max( 500, container.height() );
	var pad = Raphael("scratchpad", container.width(), height);
	...
	*/
	...
	$(pad.canvas).bind('touchstart', function(e){
		mousedown(e.originalEvent.touches[0].pageX - Khan.scratchpad.offsetLeft, e.originalEvent.touches[0].pageY - Khan.scratchpad.offsetTop, e.originalEvent);
		e.preventDefault();
	});
	$(pad.canvas).bind('touchmove', function(e){
		mousemove(e.originalEvent.touches[0].pageX - Khan.scratchpad.offsetLeft, e.originalEvent.touches[0].pageY - Khan.scratchpad.offsetTop)
		e.preventDefault();
	});
