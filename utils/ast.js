		...
		function Node (nid) {
			...
			jQuery.extend ( this, {
				...
				stringValue : function () {
					return String(nodePool[nid].val);
				} ,
		...
		jQuery.extend ( this, {
			...
			dump: function (nid) {
				...
				case this.Kind.UNARY:
					s = "[ " + node.op + ", " + this.dump(node.expr) + " ]";
					break;
				case this.Kind.BINARY:
					s = "[ " + node.op + ", " + this.dump(node.lhs) + ", " + this.dump(node.rhs) + " ]";
					break;
				default:
					s = node.val;
					break;
			...
			binaryExpr : function ( opstr, lhs, rhs ) {
				this.nodePool.push({ kind: this.Kind.BINARY, op: this.strToOp(opstr), lhs: lhs, rhs: rhs, val: void 0 });
				return this.nodePool.length-1;
			} ,
