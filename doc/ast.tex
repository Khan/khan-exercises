...
exercises. We also define a mapping from the internal AST form to several surface syntaxes useful 
for formatting and constructing ASTs. These data structures have wide applicability in the Khan 
exercise framework, and so by defining and implementing a core library we avoid duplication of 
effort and gain the benefits normally associated by software reuse.

We separate the management of ASTs from their construction and interpretation. This allows an
AST kind to be interpreted differently for different uses. Furthermore, it allows us to easily
add new node kinds without having to modify the core AST code.
...
procedures that construct and interpret them. 
